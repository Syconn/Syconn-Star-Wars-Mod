    ...
    public ActionResult<ItemStack> onItemRightClick(World worldIn, PlayerEntity playerIn, Hand handIn) {
        ...
        if(playerIn.isCrouching()){
            if (itemstack.getItem() instanceof LightsaberItem) {
                ...
                if (compound.getFloat(NBT_STATE) == 0.0F) {
                    compound.putFloat(NBT_STATE, 1.0F);
                } else if (compound.getFloat(NBT_STATE) == 1.0F) {
                    compound.putFloat(NBT_STATE, 0.0F);
                }
                System.out.println(compound.getFloat(NBT_STATE));
            }
